// Metric_histogram.h : main header file for the Metric_histogram DLL
//
...
class CMetric_histogramApp : public CWinApp
{
public:
	CMetric_histogramApp();
...
public:
	...
	MetricData(){
		for(int i=0;i<16;i++){H[i]=0;S[i]=0;V[i]=0;R[i]=0;G[i]=0;B[i]=0;}
	};
};


	class HMetric : public virtual MetricPlugin{
	...
	public:
		Counter Next();
		HMetric(Core *pCore);
		CString	GetName(){return CString("Histogramm");};
		CString	GetDescription(){return CString("Histogramm(RGBHSV)");};
	...
	public:
		ImageInputPin(Plugin* plug):ImageInPin(plug){img=NULL;};
		~ImageInputPin(){};
		Image *img;
		void Querry();
		Counter QueryLinkedPinNext();
	};
	class LOutPin: public CBIR::LogicOutPin{
	public:
